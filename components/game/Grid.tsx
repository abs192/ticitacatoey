...
export default function Grid({ size, data, pseudoData, onGridSquarePressed }) {
    ...
    for (let i = 0; i < boardSize; i++) {
        ...
        for (let j = 0; j < boardSize; j++) {
            let key = [i, j]
            cols.push(<GridSquare key={key} xo={boardData[i][j]} psuedoXO={pseudoData[i][j]} index={key} onGridSquarePressed={(key) => { onSquarePressed(key) }} />)
        }
        rows.push(
